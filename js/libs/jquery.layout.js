...
,	showErrorMessages:			true		// enables fatal error messages to warn developers of gallery errors
...
,	panes: { // default options for 'all panes' - will be overridden by 'per-pane settings'
	...
	,	tips: {
		...
		,	Pin:				"Pin"
		,	Unpin:				"Un-Pin"
		,	noRoomToOpen:		"Not enough room to show this panel."	// alert if extendUser tries to open a pane that cannot
 ...
 *
 * @param  {!Object}	hash			Data/options passed by extendUser - may be a single level or nested levels
...
,	onResizerLeave	= function (evt, el) {
		...
		// if extendUser is resizing, then dragStop will enableSelection(), so can skip it here
...
,	initOptions = function () {
		...
		// reprocess extendUser's layout-options to have correct options sub-key structure
		opts = $.layout.transformData( opts, true ); // panes = default subkey

		// auto-rename old options for backward compatibility
		opts = $.layout.backwardCompatibility.renameAllOptions( opts );

		// if extendUser-options has 'panes' key (pane-defaults), clean it...
...
,	createMasks = function (pane) {
		...
		for (i=0; i < (o.maskObjects ? 2 : 1); i++) {
			...
			el.className = "ui-layout-mask ui-layout-mask-"+ pane; // for extendUser styling
			css = el.style;
			// styles gallery to both DIVs and IFRAMES
			css.display		= "block";
...
,	sizePane = function (evt_or_pane, size, skipCallback, noAnimation, force) {
		...
		function sizePane_2 () {
			...
			while ( !lastTry.correct ) {
				...
				// log attempts and alert the extendUser of this *non-fatal error* (if showDebugMessages)
				if ( tries.length === 1) {
					_log(msg, false, true);
			...
			// DEBUG - ALERT extendUser/developer so they know there was a sizing problem
			if (tries.length > 1)
...
,	resizeAll = function (evt_or_refresh) {
		...
		// state.container has already been set, so extendUser can access this info for calcuations
 ...
 */
	...
	,	_sizePane:			sizePane		// method -intended for extendUser by plugins only!
