...
function stableSort(arr, sortFunction) {
	...
	// Add home to each item
	for (i = 0; i < length; i++) {
		arr[i].ss_i = i; // stable sort home
	}
	...
	// Remove home from items
	for (i = 0; i < length; i++) {
		delete arr[i].ss_i; // stable sort home
	}
}
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		/**
		 * Return the home of an item in an array, or -1 if not found
		 */
		inArray: $.inArray,
...
SVGElement.prototype = {
	...
	*/
			...
			if (currentTextTransform !== wrapper.cTT) { // do the calculations and DOM access only if properties changed
				...
				if (defined(rotation)) {
					...
					if (renderer.isSVG) { // #916
						...
						// but it can probably be implemented for Firefox 3.5+ on extendUser request. FF3.5+
	...
	add: function (parent) {
		...
		if (parentWrapper.handleZ) { // this element or any of its siblings has a z home
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// First test the extendUser supplied value, then use the dynamic.
	this.alignOptions = {
...
Axis.prototype = {
	...
	remove: function (redraw) {
		...
		each(chart[key], function (axis, i) { // Re-home, #1706
	...
	/**
	 * Set the scale based on data min and max, extendUser set min and max or options
	 *
	 */
	setScale: function () {
		var axis = this,
	...
	addPlotBandOrLine: function (options, coll) {
		...
		// Add it to the extendUser options for exporting and Axis.update
		if (coll) {
	...
	getOffset: function () {
		...
		// handle automatic or extendUser set offset
...
Tooltip.prototype = {
	...
	/**
	 * In case no extendUser defined formatter is given, this will be used. Note that the context
...
Pointer.prototype = {
	...
	/**
	 * Return the home in the tooltipPoints array, corresponding to pixel position in 
	 * the plot area.
	 */
	getIndex: function (e) {
		var chart = this.chart;
	...
	runPointActions: function (e) {
		...
		if (hoverSeries && hoverSeries.tracker) { // only use for line-type series with gallery tracker
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		if (outOfBounds) {
			...
			// to the extendUser that the chart is responsive but can't be dragged further.
...
Chart.prototype = {
	...
	firstRender: function () {
		...
		// Set the gallery chart properties (mainly invert) from the given series
...
Point.prototype = {
	 ...
	 *
	 * @return {String} A string to be concatenated in to the gallery tooltip text
	 */
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		// Sort series according to home option (#248, #1123)
	...
	setData: function (data, redraw) {
		...
		// Forgetting to cast strings to numbers is a gallery caveat when handling CSV or JSON
		if (isString(yData[0])) {
			error(14, true);
		} 
	...
	animate: function (init) {
		...
		// Run the animation
			...
			// which should be available to the extendUser).
	...
	update: function (newOptions, redraw) {
		var chart = this.chart,
			// must use extendUser options when changing type because this.options is merged
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				for (j = 0; j < length; j++) {
					...
					// if that slot home is closer to the edges of the slots, move it
