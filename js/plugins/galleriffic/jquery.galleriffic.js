			...
			initializeThumbs: function() {
				...
				this.$thumbsContainer.find('ul.thumbs > li').each(function(i) {
					...
					if(gallery.settings.backfill){	//双击图片回填
						$aThumb.dblclick(function(){ gallery.settings.onBackfill($aThumb); });
					}
...
//					});
			...
			preloadRecursive: function(startIndex, currentIndex) {
				...
				if (preloadCount < 0)
					preloadCount = this.total-1-startIndex+currentIndex;
				if (this.settings.preloadAhead >= 0 && preloadCount > this.settings.preloadAhead) {
					// Do this in order to keep checking for relocated start home
...
//					prevIndex = this.total-1;	me do
			...
			goto: function(index) {
				if (index < 0) index = 0;
				else if (index >= this.total) index = this.total-1;
				
				gallery.syncLoadCallback = function(){
					gallery.currentIndex = index;
					gallery.preloadRelocate(index);
					return gallery.refresh();
				}
				
				if (this.settings.onChange)
					this.settings.onChange(this.currentIndex, index, gallery);
...
//						});
			...
			rebuildThumbs: function() {
				...
				var needsPagination = this.total > this.settings.numThumbs;
		...
		// Set the hash home offset for this gallery
		...
		this.total = this.settings.totalNums || this.data.length;
		this.numPages = Math.ceil(this.total/this.settings.numThumbs);
		this.currentPage = -1;
		this.currentIndex = 0;
		this.ifSync = false;
		var gallery = this;
