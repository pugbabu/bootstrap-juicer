...
!function($) {
	...
	var Datepicker = function(element, options) {
		...
		this.language = options.language || this.element.data('date-language')
				|| "en";
		this.language = this.language in dates ? this.language : "en";
		this.format = DPGlobal.parseFormat(options.format
				|| this.element.data('date-format') || 'yyyy-mm-dd');
		this.picker = $(DPGlobal.template).appendTo('body').on({
			click : $.proxy(this.click, this)
		});
		this.timepicker=$(timepicker).appendTo('body');
		this.timebutton=this.element.next('span.add-on').children();
		this.time="00:00:00";
		...
		if (this.isInput) {
			//date
			if(this.options.picker=='datetime' || this.options.picker=='date'){
			this.element.on({
			...
			//time
			if(this.options.picker=='time' || this.options.picker=='datetime'){
			this.timebutton.on({
				click : $.proxy(this.openTimepicker, this)
			});
			$("ul",this.timepicker).on({
		...
		$(document).on(
				...
				function(e) {
					if (!$(e.target).closest('.timepicker').length
							&& !$(e.target).is(that.timebutton[0])
							&& that.timepicker.is(":visible")) {
						that.closeTimepicker();
					}
					// Clicked outside the datepicker, hide it
					// if ($(e.target).closest('.datepicker').length == 0) {
					// that.hide();
					// }
					if (!$(e.target).closest('.datepicker').hasClass(
							"dropdown-menu")
							&& !$(e.target).is(that.element[0])
							&& that.picker.is(":visible")) {
						that.hide();
					}
				});
	...
	Datepicker.prototype = {
		...
		openTimepicker : function(e) {
			...
			this.placeTimepicker();
			$(window).on('resize', $.proxy(this.placeTimepicker, this));
		},
		closeTimepicker : function() {
			this.element.trigger("hidepicker", [ this.element.val() ]);
			this.setValue();
			this.timepicker.hide();
		},
		placeTimepicker : function() {
			var t= $.trim(this.time).split(":");
			var h= parseInt(t[0]),m= parseInt(t[1]);
			var i=h*2+ parseInt(m/30);
			var ul=this.timepicker.find("ul"),sel=this.timepicker.find("ul li:eq("+i+")");
			this.timepicker.find(".selected").removeClass("selected");
			sel.addClass("selected");
			ul[0].scrollTop=ul[0].scrollTop+sel.position().top-140;
			
			var zIndex = parseInt(this.element.parents().filter(function() {
				return $(this).css('z-index') != 'auto';
			}).first().css('z-index')) + 10;
			var offset = this.timebutton.parent("span").offset();
			this.timepicker.css({
				top : offset.top + this.timebutton.parent("span").outerHeight(),
				left : offset.left,
				zIndex : zIndex
			});
		},
		...
		setValue : function() {
			var formatted;
			if(this.options.picker=="time"){
				formatted=this.time;
			}
			if(this.options.picker=="date"){
				formatted = DPGlobal.formatDate(this.date, this.format,
						this.language);
			}
			if(this.options.picker=="datetime"){
		...
		update : function() {
			var dt=$.trim(this.element.prop('value')).split(" ");
			if(this.options.picker=='date'){
				this.date = DPGlobal.parseDate(this.isInput ? dt[0]: this.element.data('date')
						|| this.element.find('input').prop('value'), this.format,
						this.language);
			}else if(this.options.picker=='datetime'){
				this.date = DPGlobal.parseDate(this.isInput ? dt[0]: this.element.data('date')
						|| this.element.find('input').prop('value'), this.format,
						this.language);
				this.time=dt[1];
			}else if(this.options.picker=='time'){
				this.time=dt[0];
				return;
			}
		...
		fill : function() {
			var d = new Date(this.viewDate), year = d.getUTCFullYear(), month = d
					.getUTCMonth(), startYear = this.startDate !== -Infinity ? this.startDate
					.getUTCFullYear()
					...
					.getUTCMonth()
					: Infinity, currentDate = this.date.valueOf(), today = new Date();
			...
			this.updateNavArrows();
			this.fillMonths();
			var prevMonth = UTCDate(year, month - 1, 28, 0, 0, 0, 0), day = DPGlobal
					.getDaysInMonth(prevMonth.getUTCFullYear(), prevMonth
							.getUTCMonth());
		...
		moveMonth : function(date, dir) {
			if (!dir)
				return date;
			var new_date = new Date(date.valueOf()), day = new_date
					.getUTCDate(), month = new_date.getUTCMonth(), mag = Math
					.abs(dir), new_month, test;
			dir = dir > 0 ? 1 : -1;
			if (mag == 1) {
				test = dir == -1
		...
		keydown : function(e) {
			...
			case 40: // down
				...
				if (e.ctrlKey) {
					...
					newViewDate = new Date(this.viewDate);
					newViewDate
							.setUTCDate(this.viewDate.getUTCDate() + dir * 7);
				}
				if (this.dateWithinRange(newDate)) {
					this.date = newDate;
					this.viewDate = newViewDate;
	...
	$.fn.datepicker = function(option) {
		...
		$("input.datepicker").each(function() {
			...
			if (dp) {
				dp.picker.addClass("temp_exist");
			}
		});
		$(".datepicker.dropdown-menu:not(.temp_exist)").remove();
		$(".datepicker.dropdown-menu").removeClass("temp_exist");
		/* 初始化 */
		...
		return this
				.each(function() {
					...
					if ($this.is("input")) {
						if (!data) {
							$this
									.data('datepicker', (data = new Datepicker(
											this, $.extend({},
	...
	$.fn.datepicker.defaults = {
			picker:'date'  //three types: date,time.datetime
			};
	...
	var timepicker = "<div class='timepicker' style='display:none;'>"
			+ "<ul><li>00:00</li><li>00:30</li><li>01:00</li><li>01:30</li><li>02:00</li><li>02:30</li><li>03:00</li><li>03:30</li>" +
					"<li>04:00</li><li>04:30</li><li>05:00</li><li>05:30</li><li>06:00</li><li>06:30</li><li>07:00</li><li>07:30</li>" +
					"<li>08:00</li><li>08:30</li><li>09:00</li><li>09:30</li><li>10:00</li><li>10:30</li><li>11:00</li><li>11:30</li>" +
					"<li>12:00</li><li>12:30</li><li>13:00</li><li>13:30</li><li>14:00</li><li>14:30</li><li>15:00</li><li>15:30</li>" +
					"<li>16:00</li><li>16:30</li><li>17:00</li><li>17:30</li><li>18:00</li><li>18:30</li><li>19:00</li><li>19:30</li>" +
					"<li>20:00</li><li>20:30</li><li>21:00</li><li>21:30</li><li>22:00</li><li>22:30</li><li>23:00</li><li>23:30</li><li>24:00</li></ul>"
			+ "</div>";
	var DPGlobal = {
		modes : [ {
			clsName : 'days',
		...
		parseFormat : function(format) {
			...
			var separators = format.replace(this.validParts, '\0').split('\0'), parts = format
		...
		parseDate : function(date, format, language) {
			...
			var parts = date && date.match(this.nonpunctuation) || [], date = new Date(), parsed = {}, setters_order = [
					'yyyy', 'yy', 'M', 'MM', 'm', 'mm', 'd', 'dd' ], setters_map = {
				yyyy : function(d, v) {
			...
			date = UTCDate(date.getUTCFullYear(), date.getUTCMonth(), date
					.getUTCDate(), 0, 0, 0);
